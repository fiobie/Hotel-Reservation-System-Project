...
<?php
    ...
    if (isset($_POST['addaccount'])) {
      ...
      $stmt = $conn->prepare("INSERT INTO account (ID, FirstName, LastName, Email, PhoneNumber, Position, Status) VALUES (?, ?, ?, ?, ?, ?, ?)");
      $stmt->bind_param("issssss", $ID, $FirstName, $LastName, $Email, $PhoneNumber, $Position, $Status);

      if ($stmt->execute()) {
        echo "<script>alert('Account added successfully!'); window.location.href=window.location.href;</script>";
      } else {
  ...
  <div class="nav-section">
    ...
    <div class="main-content">
      <h2>Account</h2>
      <p>This is for the accounts.</p>

      <button class="add-btn" onclick="document.getElementById('addRoomModal').style.display='block'">+ Add Account</button>

      <div class="table-scroll">
      <?php
      $sql = "SELECT ID, FirstName, LastName, Email, PhoneNumber, Position, Status FROM account";
      $result = $conn->query($sql);

      if ($result->num_rows > 0) {
          echo "<table id='accountTable' class='display nowrap' style='width:100%'>
                  <thead> 
                    <tr>
                        <th>Account ID</th>
                        <th>First Name</th>
    ...
    <div id="addRoomModal">
      <div class="modal-content">
        ...
        <form method="POST" action="">
          ...
          <button type="submit" name="addaccount">Save</button>
